...
const MOOD_COLORS = ['#c05a3d', '#e69a68', '#d9c15a', '#8fb08f', '#5c7a65'];
...
export function renderChart(svgEl, entries) {
  ...
  for (let m = 1; m <= 5; m++) {
    const y = yFor(m);
    svg += `<line x1="${PAD.left}" y1="${y}" x2="${W - PAD.right}" y2="${y}" stroke="#eae3d8" stroke-width="1"/>`;
  }
  ...
  if (known.length > 1) {
    ...
    svg += `<path d="${path}" fill="none" stroke="#7c9885" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>`;
  }

  points.forEach((p, i) => {
    if (p.avg === null) return;
    const x = xFor(i), y = yFor(p.avg);
    const color = MOOD_COLORS[Math.round(p.avg) - 1] || '#7c9885';
    svg += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4.5" fill="${color}" stroke="#fff" stroke-width="1.5"/>`;
  });
  ...
  points.forEach((p, i) => {
    ...
    svg += `<text x="${x.toFixed(1)}" y="${H - 6}" font-size="8.5" fill="#6b6a63" text-anchor="middle">${label}</text>`;
  });
